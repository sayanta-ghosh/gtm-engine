...
from nrv.utils.display import print_error, print_table, print_warning, spinner
...
def people(
    ...
    \b
    Examples:
        nrv search people --title "VP Sales" --domain google.com
        nrv search people --title CTO --location "San Francisco"
        nrv search people --school "IIT Kharagpur" --title "Director"
        nrv search people --past-company Mindtickle --title "VP Sales"
        nrv search people --title "Head of Growth" --provider rocketreach
        nrv search people --keyword "fintech" --title "CRO"
    ...
    Use 'nrv enrich person' to get contact details for specific people.
    """
    _require_auth()
    client = NrvClient()
    ...
    if json_out:
        from nrv.utils.display import print_json
...
def companies(
    ...
    Examples:
        ...
        nrv search companies --name "Stripe" --location "US"
        nrv search companies --domain stripe.com
    """
    _require_auth()
    client = NrvClient()
...
def _display_people_results(result: dict[str, Any]) -> None:
    ...
    click.echo("\nTip: Use 'nrv enrich person --email <email>' to get contact details.")
