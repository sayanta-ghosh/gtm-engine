"""nrv init — one-command onboarding for new users.
...
from nrv.utils.display import print_error, print_success, print_warning
...
def _find_nrv_executable() -> str:
    """Find the path to the nrv entry point for MCP server.

    Returns the command that Claude Code should use to start the MCP server.
    Prefers `nrv` CLI if available on PATH, falls back to `python3 -m`.
    """
    # Check if `nrv` is on PATH
    nrv_bin = shutil.which("nrv")
    if nrv_bin:
        return nrv_bin

    # Check if the current python has nrv installed
...
def _build_mcp_config() -> dict[str, Any]:
    ...
    nrv_bin = _find_nrv_executable()

    # If we found the `nrv` binary, use it directly
    if nrv_bin.endswith("nrv"):
        return {
            "command": nrv_bin,
            "args": ["mcp", "serve"],
        }
...
def _register_mcp_server(scope: str) -> bool:
    """Register nrv as an MCP server in Claude Code settings.
    ...
    if "nrv" in mcp_servers:
        click.echo(f"  nrv MCP server already registered in {settings_path}")
        return True
    ...
    settings["mcpServers"]["nrv"] = mcp_config
...
def _verify_server_reachable() -> bool:
    """Check if the nrv API server is reachable."""
    import httpx
...
@click.option(
    "--server-url",
    default=None,
    help="nrv server URL (default: http://localhost:8000 or configured value).",
)
def init(project: bool, skip_auth: bool, server_url: str | None) -> None:
    """Set up nrv for Claude Code in one command.

    \b
    This command:
      1. Authenticates you via Google (opens browser)
      2. Registers the nrv MCP server with Claude Code
      3. Verifies the connection works

    \b
    After running this, every new Claude Code session will have access to
    all nrv tools — search, enrichment, connections, and more.

    \b
    Examples:
        nrv init                    # Full setup (global)
        nrv init --project          # Project-level only
        nrv init --skip-auth        # Already logged in, just register MCP
        nrv init --server-url https://api.nrv.dev
    """
    click.echo()
    click.secho("  nrv — Agent-Native GTM Platform", fg="cyan", bold=True)
    click.secho("  ─────────────────────────────────", fg="cyan")
    click.echo()

    # ── Step 0: Configure server URL if provided ──────────────────────
    if server_url:
        from nrv.utils.config import set_config
    ...
    if skip_auth and is_authenticated():
        ...
        if not click.confirm("  Use existing session?", default=True):
            ...
            from nrv.cli.auth import _browser_oauth_flow
    ...
    if not is_authenticated():
        print_error("Authentication failed. Run `nrv auth login` manually.")
        sys.exit(1)
    ...
    click.echo("  • Claude will automatically have access to all nrv tools")
    click.echo("  • Try asking: \"Search for Series B SaaS companies hiring VPs of Sales\"")
    click.echo()
    click.echo("  Useful commands:")
    click.echo("    nrv status          Show auth & connection status")
    click.echo("    nrv credits balance Check your credit balance")
    click.echo("    nrv dashboard       Open the web dashboard")
    click.echo()
