...
- Actions and schemas come from Composio's API, proxied through the nrv server.
...
```
nrv_execute_action(app_id="gmail", action="GMAIL_SEND_EMAIL", params={...})
```
...
1. `nrv_list_connections()` → gmail is ACTIVE
2. `nrv_list_actions(app_id="gmail")` → sees GMAIL_SEND_EMAIL
3. `nrv_get_action_schema(action_name="GMAIL_SEND_EMAIL")` → learns exact params
4. `nrv_execute_action(app_id="gmail", action="GMAIL_SEND_EMAIL", params={...})`

## Connecting an App

**Dashboard:** `http://localhost:8000/console/{tenant_id}?tab=connections`
**CLI:** `nrv connect <app_id>` (opens browser for OAuth)
...
| `"Following fields are missing: {X}"` | Wrong param names | **You skipped Step 3.** Call `nrv_get_action_schema` |
| `"Tool X not found"` | Invalid action name | **You skipped Step 2.** Call `nrv_list_actions` |
...
The schema is the source of truth. When in doubt, call `nrv_get_action_schema`.
